...
export const WeatherDataFiveDay = async (cityName: any) => {
    const response = await fetch(`https://api.openweathermap.org/data/2.5/forecast?q=${cityName}&appid=${process.env.NEXT_PUBLIC_PUBLIC_KEY}&units=imperial`);
    const data = await response.json();
    return data;
}
